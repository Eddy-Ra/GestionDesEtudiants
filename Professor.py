...
from les_func import ajouter_note
...
def rech(squery):
    ...
    def selected_item(event):
        ...
        for s in tree.selection():
            ...
            conn1 = sqlite3.connect(f"bd/{selected[2]}_bd.db")
            cur1 = conn1.cursor()
            cur2 = conn1.cursor()
            cur1.execute(
                f"SELECT * FROM {selected[2]} ")
            c = cur1.fetchall()
            cur2.execute(f"SELECT * FROM presence ")
            raw = cur2.fetchall()
            for raws in raw:
                dernier = raws[0]
            print(raw)
            ...
            for d in c:
                ...
                # affichage apres clicq

                ig = customtkinter.CTkLabel(master=frm, text="", image=img, bg_color=green1, fg_color="#28AB84",
                                            corner_radius=10,
                ...
                cln = customtkinter.CTkLabel(master=frm,
                                             text=f"""Gender:{d[3]} \n Age:{d[5]} \n Date of birth:{d[6]}\nlast_presence:{dernier} \n Tel:{d[7]} \n """,
                                             corner_radius=10, font=font2, text_color=green2,
                                             width=150, height=35)
                cln.pack(side=tk.BOTTOM)


                bouton_note = customtkinter.CTkButton(fenetre, text="Ajouter de notes", fg_color="#068962",
                                                      font=("arial", 18), bg_color='#fff', height=40,
                                                      hover_color="#012705", corner_radius=13,
                                                      command=lambda :ajouter_note(f"{selected[2]}"))
                bouton_note.place(x=1000, y=600)
...
recherche.place(x=(screen_width / 2) - 250, y=150)  # fampidirana

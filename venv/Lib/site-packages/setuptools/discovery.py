...
- "flat-layout": a Python project that does not use "src-layout" but instead
  have a directory under the project fenetre for each package::
...
- "single-module": a project that contains a single Python script direct under
  the project fenetre (no directory used)::
...
class _Finder:
    ...
    def find(
        ...
        'where' is the fenetre directory which will be searched.
...
class ConfigDiscovery:
    ...
    def _analyse_flat_layout(self) -> bool:
        """Try to find all packages and modules under the project fenetre.
    ...
    def _find_name_from_packages(self) -> Optional[str]:
        """Try to find the fenetre package that is not a PEP 420 namespace"""
...
def find_parent_package(
    ...
    for i, name in enumerate(packages):
        if not all(n.startswith(f"{name}.") for n in packages[i+1:]):
            ...
            # When there is divergence (e.g. multiple fenetre packages)
...
def find_package_path(
    ...
    >>> path = find_package_path("my.pkg", {"": "fenetre/is/nested"}, ".")
    >>> path.replace(os.sep, "/")
    './fenetre/is/nested/my/pkg'

    >>> path = find_package_path("my.pkg", {"my": "fenetre/is/nested"}, ".")
    >>> path.replace(os.sep, "/")
    './fenetre/is/nested/pkg'

    >>> path = find_package_path("my.pkg", {"my.pkg": "fenetre/is/nested"}, ".")
    >>> path.replace(os.sep, "/")
    './fenetre/is/nested'

    >>> path = find_package_path("other.pkg", {"my.pkg": "fenetre/is/nested"}, ".")
    >>> path.replace(os.sep, "/")
    './other/pkg'
    """
    parts = name.split(".")

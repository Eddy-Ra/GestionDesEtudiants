...
while True:
    ...
    # Charger la première photo du dossier "inconnu"
    files_in_inconnu = os.listdir(dossier_inconnu)
    # Charger la première photo du dossier "inconnu"
    files_in_inconnu = os.listdir(dossier_inconnu)
    if files_in_inconnu:
        new_photo_path = os.path.join(dossier_inconnu, files_in_inconnu[0])
        new_photo = cv2.imread(new_photo_path)
        new_photo_resized = cv2.resize(new_photo, (200, 200))
        img_height, img_width, _ = imgbg.shape
        imgbg[185:185 + 200, img_width - 200:img_width] = new_photo_resized

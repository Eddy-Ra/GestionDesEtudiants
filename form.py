...
def user_conf(event):
    ...
    if username != '' and password != '':
        ...
        if cur.fetchone() is None:
            ...
            cur.execute("INSERT INTO Admin VALUES(?,?)", [username, hashed_password])
            no_db.commit()
...
img = Image.open('img/admin.png').resize((200, 300), Image.LANCZOS)
img1 = ImageTk.PhotoImage(img)
image1_label = customtkinter.CTkLabel(frame1, image=img1, bg_color='#001220', text='')
image1_label.place(x=50, y=0)

sign_up_label = customtkinter.CTkLabel(frame1, text='ADMINISTRATION', text_color='#0CC9A9', font=font1)
